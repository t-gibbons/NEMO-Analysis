...
def transport_calculations(runid, endyear, endmonth, endday, startyear=2002, startmonth=1, startday=5):
    ...
    """    
    #now lets try just plotting the across strait transport 
    fw_plot = []
    depth = fw_trans[0][0]['depth'].values
    date = fw_trans[0][0]['time_counter'].values
    print(date)
    print(depth)
    x = np.arange(0, len(ii))
    print(x)
    for f in fw_trans:
        fw_plot.append(f[0].values)
    fw_plot = np.array(fw_plot)
    print(fw_plot.shape)
    print(np.nansum(fw_plot))

    #plt.pcolormesh(x, depth, fw_plot.T)
    plt.pcolormesh(fw_plot.T)
    ax = plt.gca()
    ...
    #freshwater_transport.plot()
    #heat_transport.plot()
    #salt_transport.plot()
    volume_transport.plot()
    plt.title('Davis Strait '+runid)
    plt.ylabel('Volume Transport (Sv)')
    #plt.legend()
    #plt.savefig(figs_path+'hudson_strait_'+runid+'.png')
    plt.show()
    plt.clf()
    """
